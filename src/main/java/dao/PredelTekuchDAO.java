...
@ManagedBean
@ApplicationScoped
public class PredelTekuchDAO extends DopuskNaprDAO implements Serializable {
    ...
    public ArrayList<Napr_Tekuch_Soprotiv_Profile> getValues() {
        ...
        try {
            try (Connection conn = getDs().getConnection()) {
                PreparedStatement stat = conn.prepareStatement("SELECT predel_tekuchesti.id, materials.matname, "
                        + "predel_tekuchesti.thickness_min, predel_tekuchesti.thickness_max, predel_tekuchesti.temp,"
                        + " predel_tekuchesti.value "
                        + "FROM calculator.predel_tekuchesti INNER JOIN calculator.materials ON predel_tekuchesti.material_id = materials.id");
            ...
            Logger.getLogger(PredelTekuchDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        try {
            try (Connection conn = super.getDs().getConnection()) {
                ...
                try {
                    ...
                    String query = "INSERT INTO `calculator`.`predel_tekuchesti`\n"
                            + "(`material_id`,\n"
                            + "`thickness_min`,\n"
                            + "`thickness_max`,\n"
                            + "`temp`,\n"
        ...
        try {
            try (Connection conn = getDs().getConnection()) {
                ...
                try {
                    ...
                    String querry = "UPDATE `calculator`.`predel_tekuchesti` "
    ...
    public void deleteValue(Napr_Tekuch_Soprotiv_Profile selectedValue) {
        ...
        try {
            try (Connection conn = getDs().getConnection()) {
                ...
                stat.executeUpdate("DELETE FROM `calculator`.`predel_tekuchesti` WHERE `id`='" + selectedValue.getId() + "'");
            }
        } catch (SQLException ex) {
            Logger.getLogger(PredelTekuchDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
